...
public:
  ...
  geometry_msgs::Vector3 get_liner_accel();
  geometry_msgs::Vector3 get_angular_accel();
...
private:
  ...
  geometry_msgs::Vector3 linear_accel,angular_accel;
  ...
  void getlocalPosition_accel_CB(const geometry_msgs::AccelWithCovarianceStamped::ConstPtr &msg);
...
bool AeroDrone::takeoff()
{
  ...
  srv_takeoff.request.altitude = 1.6; //home_.geo.altitude;
...
void AeroDrone::getlocalPosition_accel_CB(const geometry_msgs::AccelWithCovarianceStamped::ConstPtr &msg)
{
    linear_accel = msg->accel.accel.linear;
    angular_accel = msg->accel.accel.angular;
}
...
void AeroDrone::watchFlightModeThread()
{
  ...
  // loacl_position_accel
  ros::Subscriber loacl_position_accel = nh_.subscribe<geometry_msgs::AccelWithCovarianceStamped>(
    topic_head_ + "/mavros/local_position/accel", 5, boost::bind(&AeroDrone::getlocalPosition_accel_CB, this, _1));
...
void AeroDrone::getWaypointsFromQGCPlan(const std::string& qgc_plan_file)
{
  try
  {
    ...
    for (auto& mi : mission_pt.get_child("mission.items"))
    {
      ...
      {
        std::cerr <<"Get waypoint info "<< e.what() << '\n';
        ROS_ERROR("Get waypoint info %s", e.what());
        std::cout << "try to get from TransectStyleComplexItem.Items" <<endl;
        for (auto& smi : mi.second.get_child("TransectStyleComplexItem.Items"))
        {
           wp.frame = smi.second.get<int>("frame");
           wp.command = smi.second.get<int>("command");
           wp.autocontinue = smi.second.get<bool>("autoContinue");
           wp.is_current = first ? true : false;
            first = false; 
            // Parameters
            std::vector<double> params;
            int param_num = 0;
            for (auto& p : smi.second.get_child("params"))
            {
              try{
                params.push_back(p.second.get<double>(""));
                param_num ++;
        ...
        std::cout << "读取完毕 " <<endl;
        // wp.frame = secmi.get<int>("frame");
        // wp.command = secmi.get<int>("command");
        // wp.autocontinue = secmi.get<int>("autoContinue");
      }
...
// Send WPs to Vehicle, get WP_list in the getWaypointsFromQGCPlan, then used in the uploadMission function
...
geometry_msgs::Vector3 AeroDrone::get_liner_accel()
{
  return linear_accel;
}

geometry_msgs::Vector3 AeroDrone::get_angular_accel()
{
...
void AeroDrone::setVelocityBody(float vx, float vy, float vz, float yaw_rate)
{
  ...
  else
  {
    ...
    cout << "pos.velocity.x" << pos.velocity.x << endl;
    cout << "pos.velocity.y" << pos.velocity.y << endl;
    cout << "pos.velocity.z" << pos.velocity.z << endl;
